          ...
          <List
            ...
            renderItem={item => (
              <List.Item
                ...
                key={item.title}
                actions={[
                  <Link to={`/${item.number}?scroll=like`}>
                    <IconText type="like-o" text={item.reactions['+1']} />
                  </Link>,
                  <Link to={`/${item.number}?scroll=comment`}>
              ...
              >
                <Skeleton loading={issuesLoading} active avatar>
                  <List.Item.Meta
                    title={
                      <Link to={`/${item.number}`} className={styles.title}>
                        {item.title}
                      </Link>
                    }
                  />
                  <Markdown
                    dataSource={
                      item.body.match(/<p>(.*?)<\/p>/) ? item.body.match(/<p>(.*?)<\/p>/)[0] : null
                    }
                  />
                  <div>
